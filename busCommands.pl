#############################################################################
# busCommands plugin by Revok
# r3
#																			
# Openkore: http://openkore.com/											
# Openkore Brazil: http://openkore.com.br/
#
#	my special thanks goes to openkore team for developing the BUS System !
#																			
...
my $networkHooks = Plugins::addHooks(['Network::stateChanged',\&init], 
										['initialized',\&init]);
...
sub bus_message_received {
	...
	if ($msg->{messageID} eq BUS_MESSAGE_ID) {
		...
		Plugins::callHook('bus_received', {message => $msg->{args}{comm}, sender => $msg->{args}{sender}}) 
												if (($char && $msg->{args}{player} eq $char->name) || $msg->{args}{player} eq "all");		
	}
}
...
sub unload {
	...
	Plugins::delHooks($networkHooks) if $networkHooks;
